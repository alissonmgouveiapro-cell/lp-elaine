...
const text = `ÁUDIO 1  —  A Ausência que Vira Dependência
...
blocks.forEach((block, i) => {
    ...
    const num_match = lines[0].match(/ÁUDIO (\d+)/);
    const num = num_match ? num_match[1] : String(i + 1);
    const num_str = num.length === 1 ? `0${num}` : num;
    
    const title = lines[0].split('—')[1].trim();
    const subtitle = lines[1].trim();
    
    const investiga = lines[2].replace('O QUE INVESTIGA:', '').trim();
    const traz = lines[3].replace('O QUE TRAZ À TONA:', '').trim();
    const cura = lines[4].replace('A CURA:', '').trim();
    const ciencia = lines[5].replace('A CIÊNCIA:', '').trim();
    const promessa = lines.length > 6 ? lines[6].replace('PROMESSA:', '').trim() : '';
    ...
    const card = `
            ...
            <div class="relative w-full glass rounded-3xl p-6 md:p-10 border border-white/10 hover:border-c-accent/30 transition-all duration-500 group overflow-hidden">
                ...
                ${promessa ? `<div class="relative z-10 mt-8 pt-6 border-t border-white/10 bg-gradient-to-r from-c-accent/5 to-transparent -mx-6 md:-mx-10 px-6 md:px-10 pb-2">
...
const full_html = `  <!-- SESSÃO 14: O DOSSIÊ COMPLETO -->
